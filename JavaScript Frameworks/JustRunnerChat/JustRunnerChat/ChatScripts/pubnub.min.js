...
a,e):c["on"+a]=e})},sa=function(){return ra("head")[0]},S=function(a,c,b){if(b)a.setAttribute(c,b);else return a&&a.getAttribute&&a.getAttribute(c)},ua=function(a,c){for(var b in c)if(c.hasOwnProperty(b))try{a.style[b]=c[b]+(0<"|width|height|top|left|".indexOf(b)&&"number"==typeof c[b]?"px":"")}catch(d){}},va=function(a){return document.createElement(a)},wa=function(){return T||U()?0:E()},ya=function(a){function c(a,b){O||(O=1,f.onerror=n,clearTimeout(u),a||!b||X(b),setTimeout(function(){a&&Y();var b=
Q(x),c=b&&b.parentNode;c&&c.removeChild(b)},z))}if(T||U()){a:{var b,d,e=function(){if(!q){q=1;clearTimeout(g);try{d=JSON.parse(b.responseText)}catch(a){return j(1)}s=1;i(d)}},s=0,q=0,k=a.timeout||1E4,g=setTimeout(function(){j(1)},k),H=a.b||t(),h=a.data||{},i=a.c||t(),l="undefined"===typeof a.g,j=function(a){s||(s=1,clearTimeout(g),b&&(b.onerror=b.onload=n,b.abort&&b.abort(),b=n),a&&H())};try{b=U()||window.XDomainRequest&&new XDomainRequest||new XMLHttpRequest;b.onerror=b.onabort=function(){j(1)};
b.onload=b.onloadend=e;l&&(b.timeout=k);h.pnsdk=xa;var m=ka(a.url,h);b.open("GET",m,l);b.send()}catch(D){j(0);T=0;a=ya(a);break a}a=j}return a}var f=va("script"),e=a.a,x=E(),O=0,u=setTimeout(function(){c(1)},a.timeout||1E4),Y=a.b||t(),k=a.data||{},X=a.c||t();window[e]=function(a){c(0,a)};a.g||(f[za]=za);f.onerror=function(){c(1)};k.pnsdk=xa;f.src=ka(a.url,k);S(f,"id",x);sa().appendChild(f);return c},Aa=function(){return!("onLine"in navigator)?1:navigator.onLine},U=function(){if(!Ba||!Ba.get)return 0;
var a={id:U.id++,send:t(),abort:function(){a.id={}},open:function(c,b){U[a.id]=a;Ba.get(a.id,b)}};return a},za="async",xa="PubNub-JS-Web/3.5.3",T=-1==navigator.userAgent.indexOf("MSIE 6");window.console||(window.console=window.console||{});console.log||(console.log=console.error=(window.opera||{}).postError||t());var Ca,V=window.localStorage;Ca={get:function(a){try{return V?V.getItem(a):-1==document.cookie.indexOf(a)?n:((document.cookie||"").match(RegExp(a+"=([^;]+)"))||[])[1]||n}catch(c){}},set:function(a,
c){try{if(V)return V.setItem(a,c)&&0;document.cookie=a+"="+c+"; expires=Thu, 1 Aug 2030 20:00:00 UTC; path=/"}catch(b){}}};var W={list:{},unbind:function(a){W.list[a]=[]},bind:function(a,c){(W.list[a]=W.list[a]||[]).push(c)},fire:function(a,c){M(W.list[a]||[],function(a){a(c)})}},Z=Q("pubnub")||0,Da=function(a){function c(){}function b(a){J&&J(a);J=n}function d(){y.time(function(a){a||b(1);setTimeout(d,i)})}function e(){fa()||b(1);setTimeout(e,z)}function s(a){var b=0;M(oa(A),function(c){if(c=A[c])b++,
...
F=a.jsonp_cb||function(){return 0},L=a.db||{get:t(),set:t()},C=a.uuid||L&&L.get(j+"uuid")||"",y={LEAVE:function(a,b){var c={uuid:C,auth:m},d=I(f),e=F();0<a.indexOf(w)||("0"!=e&&(c.callback=e),B({g:b||D,timeout:2E3,a:e,data:c,url:[d,"v2","presence","sub_key",j,"channel",N(a),"leave"]}))},history:function(a,b){var b=a.callback||b,c=a.count||a.limit||100,d=a.reverse||"false",e=a.error||t(),f=a.channel,g=a.start,i=a.end,h={},l=F();if(!f)return p("Missing Channel");if(!b)return p("Missing Callback");if(!j)return p("Missing Subscribe Key");
h.stringtoken="true";h.count=c;h.reverse=d;h.auth=m;l&&(h.callback=l);g&&(h.start=g);i&&(h.end=i);B({a:l,data:h,c:function(a){b(a)},b:e,url:[x,"v2","history","sub-key",j,"channel",N(f)]})},replay:function(a){var b=b||a.callback||t(),c=a.source,d=a.destination,e=a.stop,f=a.start,h=a.end,g=a.reverse,a=a.limit,i=F(),k={};if(!c)return p("Missing Source Channel");if(!d)return p("Missing Destination Channel");if(!l)return p("Missing Publish Key");if(!j)return p("Missing Subscribe Key");"0"!=i&&(k.callback=
i);e&&(k.stop="all");g&&(k.reverse="true");f&&(k.start=f);h&&(k.end=h);a&&(k.count=a);k.auth=m;B({a:i,c:function(a){b(a)},b:function(){b([0,"Disconnected"])},url:[x,"v1","replay",l,j,c,d],data:k})},auth:function(a){m=a;c()},time:function(a){var b=F();B({a:b,data:{uuid:C,auth:m},timeout:5*z,url:[x,"time",b],c:function(b){a(b[0])},b:function(){a(0)}})},publish:function(a,b){var b=b||a.callback||t(),c=a.message,d=a.channel,e=F();if(!c)return p("Missing Message");if(!d)return p("Missing Channel");if(!l)return p("Missing Publish Key");
if(!j)return p("Missing Subscribe Key");c=JSON.stringify(c);d=[x,"publish",l,j,0,N(d),e,N(c)];u.push({a:e,timeout:5*z,url:d,data:{uuid:C,auth:m},c:function(a){b(a);q(1)},b:function(){b([0,"Failed",c]);q(1)}});q()},unsubscribe:function(a){a=a.channel;K=0;ca=1;a=P((a.join?a.join(","):""+a).split(","),function(a){return a+","+a+w}).join(",");M(a.split(","),function(a){v&&y.LEAVE(a,0);A[a]=0});c()},subscribe:function(a,d){function e(a){a?setTimeout(c,z):(x=I(f,1),O=I(f,1),setTimeout(function(){y.time(e)},
z));s(function(b){if(a&&b.d)return b.d=0,b.m(b.name);!a&&!b.d&&(b.d=1,b.l(b.name))})}function i(){var a=F(),d=oa(A).join(",");d&&(b(),J=B({timeout:Fa,a:a,b:function(){J=n;y.time(e)},data:{uuid:C,auth:m},url:[O,"subscribe",j,N(d),a,K],c:function(a){J=n;if(!a||"object"==typeof a&&"error"in a&&!a.error)return u(a),setTimeout(c,ga);K=!K&&ca&&L.get(j)||a[1];s(function(a){a.f||(a.f=1,a.k(a.name))});ta&&(K=1E4,ta=0);L.set(j,a[1]);var b,d=(2<a.length?a[2]:P(A,function(b){return P(Array(a[0].length).join(",").split(","),
function(){return b})}).join(",")).split(",");b=function(){var a=d.shift()||X;return[(A[a]||{}).a||Y,a.split(w)[0]]};M(a[0],function(c){var d=b();d[0](c,a,d[1])});setTimeout(i,ga)}}))}var g=a.channel,d=(d=d||a.callback)||a.message,k=a.connect||t(),l=a.reconnect||t(),q=a.disconnect||t(),u=a.error||t(),D=a.presence||0,fa=a.noheresync||0,ta=a.backfill||0,Ga=a.timetoken||0,Fa=a.timeout||h,ga=a.windowing||H;ca=a.restore;K=Ga;if(!g)return p("Missing Channel");if(!d)return p("Missing Callback");if(!j)return p("Missing Subscribe Key");
M((g.join?g.join(","):""+g).split(","),function(a){var b=A[a]||{};A[X=a]={name:a,f:b.f,d:b.d,j:1,a:Y=d,k:k,l:q,m:l};D&&(y.subscribe({channel:a+w,callback:D}),!b.j&&!fa&&y.here_now({channel:a,callback:function(b){M("uuids"in b?b.uuids:[],function(c){D({action:"join",uuid:c,timestamp:G(),occupancy:b.occupancy||1},b,a)})}}))});c=function(){b();setTimeout(i,ga)};if(!v)return ba.push(c);c()},here_now:function(a,b){var b=a.callback||b,c=a.error||t(),d=a.channel,e=F(),f={uuid:C,auth:m};if(!d)return p("Missing Channel");
if(!b)return p("Missing Callback");if(!j)return p("Missing Subscribe Key");"0"!=e&&(f.callback=e);B({a:e,data:f,c:function(a){b(a,d)},b:c,url:[x,"v2","presence","sub_key",j,"channel",N(d)]})},xdr:B,ready:pa,db:L,uuid:ja,map:P,each:M,"each-channel":s,grep:ma,offline:function(){b(1)},supplant:na,now:G,unique:E,updater:la};C||(C=y.uuid());L.set(j+"uuid",C);setTimeout(e,z);setTimeout(d,i);y.time(t());g=y;g.css=ua;g.$=Q;g.create=va;g.bind=R;g.head=sa;g.search=ra;g.attr=S;g.events=W;g.init=Da;R("beforeunload",
